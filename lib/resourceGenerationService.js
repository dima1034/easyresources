...
var service = function () {
    ...
    function generateResources(modeArg, options) {
        var mode = modeArg || defaults.mode,
            worksheetName = options.worksheetName || defaults.worksheetName,
            resourceFile = options.resourceFileArg || defaults.resourceFile,
            outputPath = options.outputPathArg || defaults.output,
            excludeLocalizations = options.excludeLocalizations;
        
        return dataReceiver.receive(resourceFile, worksheetName, excludeLocalizations, mode !== 'template')
            .then(function (data) {
